...
CHUNK = 1024
RATE = 44100
SERIAL_PORT = 'COM4'  # Change to your ESP32 port
BAUDRATE = 115200
...
MIN_ACTIVATION_THRESHOLD = 10000  # Minimum amplitude for LED activation
MAX_AMPLITUDE = 60000  # Expected max FFT amplitude for loud music
            ...
            try:
                ...
                for i, band in enumerate(bands):
                    if band < np.sqrt(MIN_ACTIVATION_THRESHOLD):
                        ...
                        levels[i] = (band - np.sqrt(MIN_ACTIVATION_THRESHOLD)) / (np.sqrt(MAX_AMPLITUDE / 4) - np.sqrt(MIN_ACTIVATION_THRESHOLD)) * 8
                    else:  # Moderate to loud
                        levels[i] = 8 + (band - np.sqrt(MAX_AMPLITUDE / 4)) / (np.sqrt(MAX_AMPLITUDE) - np.sqrt(MAX_AMPLITUDE / 4)) * 8
                    levels[i] = min(16, max(0, levels[i]))
